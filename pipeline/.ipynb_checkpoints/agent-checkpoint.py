...
class Agent:
    def __init__(self, CONFIG_PATH: str, ROOT_DIR: str) -> None:
        ...
        embedding_function = self.define_embedding(device)
        
        if device != "cuda":
            self.config['repo_database']["database_type"] = "csv"
    ...
    def build_prompt(self, query: str) -> str:
        ...
        if query.startswith("#"):
            query = query.replace("#", "")
            template = "You are an {language} code assistant for solving programming problems. You are given several context, feel free to use them.\nContexts: {context}\nQuery: {query} \nAnswer:"
        ...
        if self.config["agent"]["trim_context"]:
            context = self.trim_context(contexts, query, template)
